...
function Accounts(){
	...
	function importData(){
		...
		html='<p id="exportTable" style="display:none">'
			+'account id,account name,account class,account balance'
			+'</p>'
	...
	function bacaData(isi){
		...
		for (var x=1;x<baris;x++){ 
			...
			if (e.length>=jumlah){
				...
				arr.account_balance=baris_ke[3];
	...
	function importExecute(){
		...
		for (var i=0;i<dataImport.length;i++){
			obj = {
				...
				"account_balance":dataImport[i].account_balance
			}
